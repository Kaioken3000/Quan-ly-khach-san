...
@section('content')
    ...
    <form action="{{ route('users.update', $user->id) }}" method="POST">
        ...
        <div class="mb-3">
            ...
            <input type="email" name="email" class="form-control" id="email" placeholder="VD: Admin@gmail.com"
                value="{{ $user->email }}" />
            @error('email')
        ...
        <div class="mb-3">
            ...
            <input type="text" name="username" class="form-control" id="username" placeholder="VD: Admin"
                value="{{ $user->username }}" />
            @error('username')
                <div class="alert alert-danger" user="alert">{{ $message }}</div>
            @enderror
        ...
        <div class="mb-3">
            ...
            <input type="text" name="sdt" class="form-control" id="sdt" placeholder="VD: Admin"
                value="0{{ $user->sdt }}" />
            @error('sdt')
                <div class="alert alert-danger" user="alert">{{ $message }}</div>
            @enderror
    ...
    <form action="/updateUserid" method="POST">
        ...
        <button type="submit" class="btn btn-primary mb-3">Cập nhật nhân viên</button>
    </form>
@endsection
